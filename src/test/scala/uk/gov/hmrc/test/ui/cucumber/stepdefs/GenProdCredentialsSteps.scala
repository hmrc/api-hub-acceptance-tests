...
import uk.gov.hmrc.test.ui.pages.application.{GenProdCredentialsPage}
import uk.gov.hmrc.test.ui.pages.{Journeys, SignInPage, StrideSignInPage}
import uk.gov.hmrc.test.ui.pages.{Journeys, Robot}
import uk.gov.hmrc.test.ui.utilities.SharedState

class GenProdCredentialsSteps extends BaseStepDef {

  Then("""the user confirms generation production credentials""") { () =>

    GenProdCredentialsPage(sharedState.application.id).clickconfirmandcontinue()
